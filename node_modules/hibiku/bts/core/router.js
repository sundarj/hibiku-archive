...
const exts = settings.engine.extension;
Promise.each(settings.engine.views, function(view) {
    return utils.walk(view).then(function(files) {
        files.forEach(function (file) {
            ...
            if (~file.indexOf(settings.root)) {
                file = file.replace(exts.hibiku, '');
                routes.render.push(file.replace(settings.root, ''));
            } else {
                const endsWithHibiku = file.indexOf(exts.hibiku) === (file.length - exts.hibiku.length);
                if (endsWithHibiku) {
                    file = file.replace(exts.hibiku, '');
                    viewFiles.push(file.replace(settings.user.root, ''));
                } else {
                    file = file.replace(exts.plain, '');
                }
    ...
    routes.render.forEach(function (ro) {
        ...
        if (ro.indexOf('/bts/') === 0) {
            ...
            if (parts.length > 1) {
                // hook for folders (ie. path/to/filename instead of just filename)
                hook = parts.join(path.sep); 
            }
        }
