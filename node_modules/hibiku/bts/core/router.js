...
Promise.each(config.engine.views, function(view) {
    return utils.walk(view).then(function(files) {
        files.forEach(function(file) {
            file = file.replace(config.engine.extension, '');
            if (file.indexOf(config.root) > -1) {
                routes.push(file.replace(config.root, ''));
            } else {
                viewFiles.push(file.replace(config.user.root, ''));
    ...
    map['/'] = config.user.options.frontPage;
    
    const api = require('./api');
    api.use(app, require('koa-route'));
    
    app.use(serve(config.root));
    app.use(serve(config.user.root));
