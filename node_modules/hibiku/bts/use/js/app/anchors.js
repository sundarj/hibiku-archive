define(function(require) {
    ...
    nest.qs(hb.selectors.files).delegate('click', 'li', function(click) {
        ...
        [].forEach.call(document.querySelectorAll('.is'), function (is) {
            is.classList.remove('is');
        });
        click.target.classList.add('is');
        ...
        nest.request('api', {
            ...
            anchors.forEach(function(anchor) {
                if (document.getElementById(escapeAnchor(anchor)))
                    return;
                ...
                var isArea = / area$/.test(anchor);

                var field = document.createElement( isArea ? 'div' : 'input' );
                field.className = 'hb-field hb-field-' + ( isArea ? 'area' : 'inline' );
                field.id = field.name = label.htmlFor = escapeAnchor(anchor);
                label.textContent = field.dataset.placeholder = field.placeholder = anchor.replace(/ area$/, "");
            ...
            nest.request('api/post/get', {
                ...
                fields.forEach(function (field) {
                    console.log(escapeAnchor(field.anchor));
                    var input = document.getElementById(escapeAnchor(field.anchor));
                    console.log(input);
                    input.value = input.innerHTML = field.content;
                });
            });
        ...
        var values = {
                ...
                fields: nest.qs(hb.selectors.field).map(function(field) {
                    return {
                        ...
                        "content": field.innerHTML || field.value
                    };
                })
        };
